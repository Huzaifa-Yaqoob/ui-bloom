...
Be sure to import the following `CSS`, as it includes a utility class used to hide the `scrollbar` within the component. If you also want to hide the scrollbar in the `input` area where selected values are displayed, you can add the same `CSS` to your sglobal or main stylesheet.
...
Run the following command in termianal to add these components in your project.
...
This is a **simple select component**. You just need to provide the `options` prop, and the component will render the selection list based on those options.
...
You can now easily use it within your code as needed.
...
You can import it like this and use it to convert group options to single level
